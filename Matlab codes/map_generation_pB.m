...
list_B = (0.01:0.03:0.53); % radius
list_l = [20 15 10 7.5 5 2 1.5 1 0.75 0.5 0.2 0.1 0.075 0.05 0.025].*1e-3;
%list_a = (1.5:0.1:2.9).*1e-3
%list_l = (0.05:0.06:0.9).*1e-3

a = 1e-3;
D = 2*a;
A = pi*a^2;

for p_B=1:length(list_B)
    p_B
    for p_l=1:length(list_l)
        ...
        csvwrite("fitting_data_maxwell_error_v_B_" + list_B(p_B) + "_l_" + list_l(p_l) + ".txt", [v_counts_maxwell; f_est_maxwell]);
        f_est_kappa = Counts_kappa./(G*delta_v*v_counts_kappa.^3);
        csvwrite("fitting_data_kappa_error_v_B_" + list_B(p_B) + "_l_" + list_l(p_l) + ".txt", [v_counts_kappa; f_est_kappa]);
    end
end
