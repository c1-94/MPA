...
f_est_kappa = Count_pixel./(G*delta_v*v_counts.^3);
csvwrite('fitting_data_kappa_SIMION.txt', [v_counts; f_est_kappa]);

K_kappa = (2/(pi*(2*kappa-3)))^(3/2)*gamma(kappa+1)/gamma(kappa-1/2);

figure()
semilogy(v_counts, f_est_kappa, 'r-*', 'LineWidth', 2);
hold on;
semilogy(v_counts, (n/(vt^3))*K_kappa.*(1+(2/(2*kappa-3)).*((v_counts-v0).^2/vt^2)).^(-kappa-1), 'b--', 'LineWidth', 2);
ylim([1e-11 1e-7])
