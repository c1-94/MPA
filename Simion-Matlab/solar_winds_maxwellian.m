...
fast = csvread("maxwellian_700e3_3e6_2e5.csv");
bulk = csvread("maxwellian_500e3_5e6_1e5.csv");
slow = csvread("maxwellian_400e3_5_5e6_8e4.csv");
...
f_est_maxwell_bulk = Count_pixel_bulk./(G*delta_v*v_counts_bulk.^3);
f_est_maxwell_fast = Count_pixel_fast./(G*delta_v*v_counts_fast.^3);
f_est_maxwell_slow = Count_pixel_slow./(G*delta_v*v_counts_slow.^3);
csvwrite('fitting_data_maxwell_SIMION.txt', [v_counts_bulk; f_est_maxwell_bulk]);

vt_fast = ((2*kb*2e5)/m)^(1/2);
vt_slow = ((2*kb*0.8e5)/m)^(1/2);

figure()
semilogy(v_counts_bulk, f_est_maxwell_bulk, 'r-*', 'LineWidth', 2);
hold on;
set(gca,'FontSize',14);
semilogy(v, (5e6/(pi^(3/2)*vt^3)).*exp(-(v-v0).^2/vt^2), 'b--', 'LineWidth', 2);
semilogy(v_counts_fast, f_est_maxwell_fast, 'r-*', 'LineWidth', 2);
semilogy(v, (3e6/(pi^(3/2)*vt_fast^3)).*exp(-(v-700e3).^2/vt_fast^2), 'b--', 'LineWidth', 2);
semilogy(v_counts_slow, f_est_maxwell_slow, 'r-*', 'LineWidth', 2);
semilogy(v, (5.5e6/(pi^(3/2)*vt_slow^3)).*exp(-(v-400e3).^2/vt_slow^2), 'b--', 'LineWidth', 2);
ylim([1e-11 1e-7])
...
csvwrite("fitting_data_maxwell_SIMION_bulk.txt", [v_counts_bulk; f_est_maxwell_bulk]);
csvwrite("fitting_data_maxwell_SIMION_fast.txt", [v_counts_fast; f_est_maxwell_fast]);
csvwrite("fitting_data_maxwell_SIMION_slow.txt", [v_counts_slow; f_est_maxwell_slow]);




% figure()
% semilogy(pixel_size_list, er_density, 'LineWidth', 2);
% hold on;
% semilogy(pixel_size_list, er_vt, 'LineWidth', 2);
% title("relative error on parameters with pixel size");
% xlabel("pixel size (m)");
% ylabel("e_r (%)");
% legend("density", "vt");
